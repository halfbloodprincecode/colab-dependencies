  ...
  def getCallbacks(self, params, fit=[], evaluate=[], predict=[], indexing=None):
    cb = {
      'fit': [
        ...
        keras.callbacks.ModelCheckpoint(filepath=f'{params["DIR_ROOT"]}/experimental/{params["model"]}/weights/'+'{int(epoch):09d}, {val_loss:.2f}/weights', monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=True, mode='min', save_freq='epoch', options=None, initial_value_threshold=None),
