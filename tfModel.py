  ...
  def getCallbacks(self, params, fit=[], evaluate=[], predict=[], indexing=None):
    cb = {
      'fit': [
        ...
        keras.callbacks.ModelCheckpoint(filepath=params["DIR_ROOT"]+'/experimental/'+params["model"]+'/weights/{epoch:02d}, {val_loss:.2f}/weights', monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=True, mode='min', save_freq='epoch', options=None, initial_value_threshold=None),
